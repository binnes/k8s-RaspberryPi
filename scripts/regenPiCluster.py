...
def createSDimage(config, sysType, host):
    ...
    if not os.path.exists(mountPoint):
        os.mkdir(mountPoint)
    cmdline = 'dwc_otg.lpm_enable=0 console=serial0,115200 console=tty1 root=/dev/nfs nfsroot={}:{}/{},vers=3 rw ip={}:{}:{}:{}:{}:eth0:off elevator=deadline rootwait'.format(config["testMachines"]["NFSserver"], fsRoot, host["name"], host["IP"], config["testMachines"]["NFSserver"], config["testMachines"]["network"]["routerIP"], config["testMachines"]["network"]["netmask"], host["name"])
