...
def prepareKubeHost(config, host):
    ...
    runRemoteCommand(host, "sudo -E sh -c apt-get update -qq >/dev/null")
    runRemoteCommand(host, "sudo -E sh -c apt-get install -y -qq apt-transport-https ca-certificates curl >/dev/null")
    runRemoteCommand(host, """sudo -E sh -c curl -fsSL \"https://download.docker.com/linux/raspbian/gpg\" | apt-key add -qq - >/dev/null""")
    runRemoteCommand(host, """sudo -E sh -c echo \"deb [arch=armhf] http://HTTPS///download.docker.com/linux/raspbian stretch edge\" > /etc/apt/sources.list.d/docker.list""")
    runRemoteCommand(host, "sudo -E sh -c apt-get update -qq >/dev/null")
    runRemoteCommand(host, "sudo -E sh -c apt-get install -y -qq --no-install-recommends docker-ce >/dev/null")
    runRemoteCommand(host, "sudo -E sh -c docker version")
